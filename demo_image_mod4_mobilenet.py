...
from model_mod4_mobnet import get_testing_model
...
def process (input_image, params, model_params):
    ...
    return canvas, heatmap_avg, paf_avg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imdir', type=str, required=False, help='input image', default='./sample_images')
    ...
    fmodelBest = "weights_mobilenet4_best_a{}_s{}.h5".format(paramAlpha, paramNumStages)
    keras_weights_file = '{}/{}'.format(args.modelDir, fmodelBest)

    # load model
    print('[*] Load trained model from [{}]'.format(keras_weights_file))
    model = get_testing_model(pstages=paramNumStages, palpha=paramAlpha, isNormalizeData=True)
    ...
    for ii, input_image in enumerate(lstImages):
        ...
        canvas, ret_hmp, ret_paf = process(input_image, params, model_params)
        ...
        plt.imshow(canvas)
        plt.subplot(1, 4, 3)
        plt.imshow(np.sum(ret_hmp[:, :,:-1], axis=-1))
        plt.title('heatmap')
        plt.subplot(1, 4, 4)
        plt.imshow(np.sum(ret_paf[:, :, :-1], axis=-1))
        plt.title('paf')
        plt.show()
