...
def stage1_block_mobilenet(x, num_p, branch, weight_decay, palpha = 1.0, depth_multiplier=1):
    # Block 1
    x = _depthwise_conv_block_mod(x, 256, palpha, depth_multiplier, block_name="Mconv1_stage1_L%d" % branch)
    x = _depthwise_conv_block_mod(x, 256, palpha, depth_multiplier, block_name="Mconv2_stage1_L%d" % branch)
    # x = conv(x, 128, 3, "Mconv1_stage1_L%d" % branch, (weight_decay, 0))
    # x = relu(x)
    # x = conv(x, 128, 3, "Mconv2_stage1_L%d" % branch, (weight_decay, 0))
    # x = relu(x)
...
def stageT_block_mobilenet(x, num_p, stage, branch, weight_decay, palpha = 1.0, depth_multiplier=1):
    x = _depthwise_conv_block_mod(x, 256, palpha, depth_multiplier, block_name="Mconv1_stage%d_L%d" % (stage, branch))
    x = _depthwise_conv_block_mod(x, 256, palpha, depth_multiplier, block_name="Mconv2_stage%d_L%d" % (stage, branch))
    # Block 1
    # x = conv(x, 128, 5, "Mconv1_stage%d_L%d" % (stage, branch), (weight_decay, 0))
    # x = relu(x)
    # x = conv(x, 128, 3, "Mconv2_stage%d_L%d" % (stage, branch), (weight_decay, 0))
